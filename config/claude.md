...
- Treat a defensible English translation as the destination, with disclosed
  sign-value confidence and a tested language-family identification as the
  necessary preceding steps.
- Follow the priority order in `research-department.md`: sign-value
  confidence disclosure first, held-out language-family hypothesis testing
...
- Keep the homepage understandable to a typical 10th-grade reader and
  ...
  a language has been identified or a translation has occurred when it has
  not.
- Never download inscription images, the GORILA corpus, or other bulk
  third-party files without explicit user authorization, the same standing
  rule the sibling projects follow for their own primary material.
- Never treat a borrowed Linear-B phonetic value as confident for a sign
  without checking its classification in the sign-value confidence atlas
  (SQ-2) first.
