# Contributing to the Linear A Collective
...
canonicalization (SQ-1) or the sign-value confidence atlas (SQ-2), since
both block everything downstream.
...
1. **Clone or fork** the repository:
   ```
   git clone https://github.com/SoylentAquamarine/linear-a-collective.git
   ```
...
4. **Pick one bounded task** — do not invent a new primary direction for
   ...
   - Corpus canonicalization research (`config/sidequests.md` SQ-1) —
     ...
     corpora of Linear A. This is the single most valuable thing a new
     contributor can do right now.
   - Sign-value confidence work (`config/sidequests.md` SQ-2) — for a named
     subset of signs, researching whether their Linear-B-derived phonetic
     value is directly attested, inferred, or unknown for Linear A.
   ...
   - **Do not**: edit `knowledge-base/state.md` directly (it only changes
     via PR and review, same rule for everyone), bulk-download inscription
     images or the GORILA corpus without explicit user authorization, or
     claim "independently reproduced" without actually rerunning the
...
(SQ-1, corpus canonicalization, and SQ-2, the sign-value confidence atlas,
are the current blocking priorities) or a named follow-up in a recent log
or comms entry — do not touch knowledge-base/state.md directly. Do the
work, write it up with the same disclosure standard as the rest of this
repo (checksums, honest null results, no unfalsifiable claims), and open a
pull request. Work continuously and do not wait for a response before
starting — the lead agent reviews on its own schedule and will not block
you, and you should not block it.
```
...
- Contributions that assert a language identification or translation
  without meeting
  [`methods/falsification-standard.md`](methods/falsification-standard.md).
- Bulk downloads of inscription images, the GORILA corpus, or other files
  without explicit authorization — check recent logs for what's already
  been authorized before fetching anything new.
...
- A candidate language match built from a handful of resembling
  sign-sequences without corpus-wide, held-out validation — the specific,
  well-documented failure mode this project's `agents/skeptic.md` exists to
  catch.
- Treating a borrowed Linear-B phonetic value as confident for a
  Linear-A-only sign without checking `agents/linguist.md`'s and SQ-2's
  confidence classification first.
