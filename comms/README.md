...
Any change to source data, normalization, transliteration, sign-value
confidence classification, or a shared metric must be treated as a
dependency-graph change, not as a one-file correction:
...
This rule applies with particular force to the sign-value confidence atlas
(`config/sidequests.md` SQ-2): a revised confidence tier for even a small
number of signs can change which downstream language-family results are
trustworthy, so a revision there must be propagated, not patched around.
